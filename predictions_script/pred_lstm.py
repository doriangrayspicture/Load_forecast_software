...
def run_prediction(data_file, predict_date):
    ...
    model_dir = "resources/LSTM_unified_basic_lag15"
    model = load_model(os.path.join(model_dir, "lstm_uni.h5"), compile=False)
    ...
    if df_lag.shape[0] < 96 * 15:
        raise ValueError(f"Insufficient data to predict {predict_date}. Expected 15 full days, got {df_lag['Date'].nunique()} days.")
